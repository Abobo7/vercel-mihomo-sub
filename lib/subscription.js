...
function mapNetwork(net) {
  ...
  // Mihomo is generally more stable with ws than httpupgrade.
  if (n === 'httpupgrade') return 'ws';
  if (['ws', 'grpc', 'h2', 'http'].includes(n)) return n;
  return 'tcp';
}
...
function vmessToMihomo(vmess) {
  ...
  if (network === 'ws' && (vmess.path || vmess.host)) {
    proxy['ws-opts'] = {
      path: vmess.path || '/',
